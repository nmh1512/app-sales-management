...
export default function InventoryDisposalFormPage() {
  const navigate = useNavigate();
  const [isProductModalOpen, setIsProductModalOpen] = useState(false);
  const [products] = useState([
    {
      id: 1,
      code: "cucapsacxiaomi67w",
      name: "Bộ Củ Cáp Sạc Xiaomi 67W",
      unit: "Cái",
      disposeQty: 1,
      costPrice: 52000,
    },
  ...
  const totalQty = products.reduce((acc, p) => acc + p.disposeQty, 0);
  const totalValue = products.reduce(
    (acc, p) => acc + p.disposeQty * p.costPrice,
    0,
  );
      ...
      <div className="h-14 border-b bg-card px-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => navigate("/inventory-disposals")}
            ...
            <Table>
              <TableHeader className="bg-muted/30 sticky top-0 z-10">
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead
                    className="text-xs font-bold py-3 text-center"
                    title="Số lượng hủy"
                  >
                    SL HỦY
                  </TableHead>
                ...
                {products.map((item, index) => (
                  <TableRow key={item.id} className="hover:bg-muted/20">
                    ...
                    <TableCell className="py-4 text-right font-bold text-sm text-primary">
                      {(item.disposeQty * item.costPrice).toLocaleString()}
                    </TableCell>
            ...
            <div className="space-y-4 pt-2">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-muted-foreground flex items-center gap-1">
                  Ngày xuất hủy <span className="text-destructive">*</span>
                </label>
              ...
              <div className="pt-4 border-t space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-sm">Tổng số lượng hủy</span>
                ...
                <div className="flex items-center justify-between pt-2">
                  <span className="text-sm font-bold">Tổng giá trị hủy:</span>
                  <span className="font-bold text-lg text-destructive tracking-tight">
